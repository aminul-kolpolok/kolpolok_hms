...
# from conf.custom_hrms_root.general_hospital_v_03.blood_bank.blood_bank import _
# from odoo.addons.kolpolok_hms.blood_bank.blood_bank import _
...
class PatientInfo(models.Model):
    ...
    age_manual = fields.Integer(compute='compute_age', string="Manual Entry Age", store=True, tracking=True, inverse='_inverse_compute_date_of_birth')
    age = fields.Char(string="Patient Age",store=True, tracking=True)
    ...
    # patient_create_date = fields.Datetime.now()
    ...
    def compute_age(self):
        for rec in self:
            ...
            if rec.date_of_birth:
                rec.age_manual = today.year - rec.date_of_birth.year
            else:
                rec.age_manual =1

    @api.depends('age_manual')
    def _inverse_compute_date_of_birth(self):
        today = date.today()
        for rec in self:
             rec.date_of_birth = today - relativedelta(years=rec.age_manual)
    #
    #
    #         # else:
    ...
    def button_patient_card_send_by_email(self):
        mail_template = self.env.ref('kolpolok_hms.patient_card_email_template')
        for patient in self:
